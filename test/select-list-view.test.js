...
describe("SelectListView", () => {
  ...
  it("focus", async () => {
    ...
    assert.equal(
      document.activeElement.closest("soldat-text-editor"),
      selectListView.refs.queryEditor.element
    );
    ...
    selectListView.refs.queryEditor.element.dispatchEvent(
      new FocusEvent("blur")
    );
    assert.equal(cancelSelectionEventsCount, 1);
  ...
  it("keyboard navigation and selection", async () => {
    ...
    const selectListView = new SelectListView({
      ...
      didChangeSelection: item => {
        selectionChangeEvents.push(item);
      },
      didConfirmSelection: item => {
        selectionConfirmEvents.push(item);
      }
    });
    ...
    assert.equal(
      selectListView.element.querySelector(".selected").textContent,
      items[0].name
    );
    ...
    assert.throws(() => {
      selectListView.selectItem(null);
    });
  ...
  it("confirming an empty selection", async () => {
    ...
    const selectListView = new SelectListView({
      ...
      didConfirmEmptySelection: item => {
        emptySelectionConfirmEventsCount++;
      },
      didCancelSelection: () => {
        selectionCancelEventsCount++;
      }
    });
  ...
  it("default filtering", async () => {
    ...
    await selectListView.selectNext();
    assert.equal(
      selectListView.refs.items.innerText,
      "Grace\nJohnathan\nJoanna"
    );
    assert.equal(selectListView.getSelectedItem(), items[1]);
  ...
  it("loading message", async () => {
    ...
    assert(!selectListView.refs.emptyMessage);
    assert.equal(
      selectListView.refs.loadingMessage.textContent,
      "loading message"
    );
